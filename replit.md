...
- **Server Directory Structure**:
  ...
  - `server/industries/` — Industry profiles, industry detection, manufacturing root causes/KPIs/vocabulary. `manufacturing-root-cause-library.ts` is the **authoritative** manufacturing root cause source: 104 entries (25 primary Tier-1 + 79 secondary Tier-2) across 4M categories with `ManufacturingRootCauseEntry` type (`id`, `name`, `category`, `tier: "primary"|"secondary"`, `triggers`, `supportingSignals`, `description`). At server startup, `bulk-analyzer.ts` adapts these 104 entries to V2 shape and registers them via `registerManufacturingV2Data()` in `shared/root-cause-library.ts` for scoring. Legacy `manufacturing-root-causes.ts` retained only for `ManufacturingRootCause` type used by consulting engine. `manufacturing-vocabulary.ts` exports typed `ManufacturingSignal[]` (149 signals across 4M categories with ~1200 phrase variations) plus backward-compatible `manufacturingVocabulary: Record<string, string[]>`
