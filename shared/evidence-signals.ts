...
const SIGNAL_RULES: SignalRule[] = [
  ...
  },
  {
    ...
    terms: [
      ...
      "below production target",
      "last-minute change",
      "plan change",
      "schedule disruption",
      "line efficiency",
      "production efficiency drop",
...
const METRIC_PATTERNS: MetricPattern[] = [
  ...
  {
    regex: /(?:COPQ|cost\s+of\s+poor\s+quality|cost\s+of\s+quality|COQ|quality\s+cost)\s*(?:[:=]\s*)?(?:\$|RM|USD|MYR)?\s*(\d[\d,]*(?:\.\d{2})?)/gi,
    formatter: (m) => `COPQ ${m[1]}`,
    category: "Money",
  },
  {
    regex: /(?:inventory\s+turnover|stock\s+turns?|inventory\s+turns?)\s*(?:[:=]\s*)?(\d+(?:\.\d+)?)\s*(?:times?|x)?/gi,
    formatter: (m) => `Inventory turnover ${m[1]}x`,
    category: "Materials",
  },
  {
    regex: /(?:warranty\s+(?:cost|claim|expense))\s*(?:[:=]\s*)?(?:\$|RM|USD|MYR)?\s*(\d[\d,]*(?:\.\d{2})?)/gi,
    formatter: (m) => `Warranty cost ${m[1]}`,
    category: "Money",
  },
  {
    regex: /(?:energy\s+cost|utility\s+cost|electricity\s+cost|power\s+cost)\s*(?:[:=]\s*)?(?:\$|RM|USD|MYR)?\s*(\d[\d,]*(?:\.\d{2})?)/gi,
    formatter: (m) => `Energy/utility cost ${m[1]}`,
    category: "Money",
  },
  {
    regex: /(?:overtime\s+premium|OT\s+premium)\s*(?:[:=]\s*)?(?:\$|RM|USD|MYR)?\s*(\d[\d,]*(?:\.\d{2})?)/gi,
    formatter: (m) => `Overtime premium ${m[1]}`,
    category: "Money",
  },
  {
    regex: /(?:maintenance\s+cost)\s*(?:increased?|up|rose|higher)\s*(?:by\s+)?(\d+(?:\.\d+)?)\s*%/gi,
    formatter: (m) => `Maintenance cost increase ${m[1]}%`,
    category: "Money",
  },
  {
    regex: /(?:purchase\s+price\s+variance|PPV|material\s+cost\s+variance|price\s+variance)\s*(?:[:=]\s*)?(?:\$|RM|USD|MYR)?\s*(\d[\d,]*(?:\.\d{2})?)/gi,
    formatter: (m) => `PPV/price variance ${m[1]}`,
    category: "Money",
  },
];
...
const EVENT_PATTERNS: EventPattern[] = [
  ...
  {
    regex: /(?:operator\s+error|human\s+error|worker\s+error|operator\s+mistake)/gi,
    formatter: () => "Operator/human error detected",
    category: "Manpower",
  },
  {
    regex: /(?:communication\s+breakdown|miscommunication|poor\s+communication|lack\s+of\s+communication)/gi,
    formatter: () => "Communication breakdown",
    category: "Manpower",
  },
  {
    regex: /(?:shift\s+handover|handover\s+incomplete|no\s+handover|poor\s+handover|handover\s+gap)/gi,
    formatter: () => "Shift handover issue",
    category: "Manpower",
  },
  {
    regex: /(?:schedule\s+deviation|schedule\s+miss(?:ed)?|schedule\s+not\s+met|off\s+schedule|schedule\s+adherence\s+(?:low|poor|below|missed))/gi,
    formatter: () => "Schedule deviation",
    category: "Machinery",
  },
  {
    regex: /(?:rush\s+order|expedite\s+order|expediting|hot\s+job|emergency\s+order|unplanned\s+order)/gi,
    formatter: () => "Rush/expedite order",
    category: "Materials",
  },
  {
    regex: /(?:partial\s+delivery|short\s+delivery|short\s+shipment|under.?delivery|delivery\s+shortfall)/gi,
    formatter: () => "Partial/short delivery",
    category: "Materials",
  },
  {
    regex: /(?:return\s+to\s+vendor|\bRTV\b|supplier\s+rejection|goods\s+returned\s+to\s+supplier)/gi,
    formatter: () => "Return to vendor",
    category: "Materials",
  },
  {
    regex: /(?:safety\s+stock\s+(?:breach|depleted|low|below|violated)|below\s+safety\s+stock)/gi,
    formatter: () => "Safety stock breach",
    category: "Materials",
  },
  {
    regex: /(?:reorder\s+point\s+(?:breach|breached|below|missed)|below\s+reorder\s+point)/gi,
    formatter: () => "Reorder point breach",
    category: "Materials",
  },
  {
    regex: /(?:cycle\s+count\s+(?:discrepancy|variance|error|mismatch)|physical\s+count\s+(?:mismatch|error|variance)|stock\s+count\s+error)/gi,
    formatter: () => "Cycle count discrepancy",
    category: "Materials",
  },
];
